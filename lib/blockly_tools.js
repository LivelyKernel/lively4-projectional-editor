...
export function updateBlockConnections(block) {
	...
	if (prevConnected || nextConnected) {	
		block.setOutput(false, block.lpe_aliases);
	} else if (outConnected) {
		block.setPreviousStatement(false, block.lpe_aliases);
		block.setNextStatement(false, block.lpe_aliases);		        
	} else {		   
		block.setOutput(true, block.lpe_aliases);
		block.setPreviousStatement(true, block.lpe_aliases);
		block.setNextStatement(true, block.lpe_aliases);
	}
